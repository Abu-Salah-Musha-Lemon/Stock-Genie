...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
    ...
    <form method="post" action="{{ route('update', $editEmp->id) }}" 
        ...
        <!-- Profile Name Field -->
        <div>
            <x-input-label for="phone" :value="__('Profile Name')" />
            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full shadow-sm border-gray-300 rounded-md" :value="old('name', $editEmp->name)" />
...
<div>
    ...
    <!-- Preview Image -->
    <div class="mt-2">
    <img id="photoPreview" src="{{ old('photo', $editEmp->photo) }}" alt="Photo Preview" class="w-full h-auto rounded-md" style="display: {{ old('photo', $editEmp->photo) ? 'block' : 'none' }}; width: 180px; height: 180px; object-fit: cover;" />
</div>
